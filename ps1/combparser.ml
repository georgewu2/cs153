...
let rec make_base_exp_parser (():unit) : (token, exp) parser =
...
and make_aexp_parser (():unit) : (token, exp) parser = 
  ...
  alts [aexp_parser; make_base_exp_parser ()]
...
(* then checks for PLUS and MINUS *)
...
(* then checks for (IN)EQUALITY tokens *)
...
(* finally checks for ASSIGN, this is the base expression parser *)
...
(* This is our total expression parser *)
and make_exp_parser (():unit) : (token, exp) parser = 
  make_gexp_parser ()

(* This is our program parser, it looks for a list of statements *)
...
and make_stmt_parser (():unit) : (token, stmt) parser =
  ...
  let exp_parser = seq (make_exp_parser (), satisfy (fun t -> t == SEMI)) in
...
(* This closed statement solves the dangling else,
 * in a cstmt, there is no if-then *)
...
and make_while_parser ((): unit) : (token, stmt) parser = 
  ...
  let all_parser = seq(while_parser, lazy_seq(lazy (make_exp_parser ()), lazy (make_stmt_parser ()))) in
...
and make_for_parser ((): unit) : (token, stmt) parser = 
  ...
  let rest_parser = lazy_seq (lazy (make_exp_parser ()), lazy (rest_parser)) in
  let rest_parser = lazy_seq (lazy (satisfy (fun t -> t == SEMI)), lazy (rest_parser)) in
  let rest_parser = lazy_seq (lazy (make_exp_parser ()), lazy (rest_parser)) in
  let rest_parser = lazy_seq (lazy (satisfy (fun t -> t == SEMI)), lazy (rest_parser)) in
  let rest_parser = lazy_seq (lazy (make_exp_parser ()), lazy (rest_parser)) in
  let for_parser = seq (satisfy (fun t -> t == FOR), 
    lazy_seq(lazy (satisfy (fun t -> t == LPAREN)), lazy rest_parser)) in
  map (fun (_, (_, (e1, (_, (e2, (_, (e3, (_, s)))))))) -> 
    (For (e1, e2, e3, s), dummy_pos)) for_parser
(* Handles if-else, going into a cstmt in the if statement *)
and make_if_else_parser (():unit) : (token, stmt) parser = 
  ...
  let rest_parser = lazy_seq(lazy (make_exp_parser ()), lazy rest_parser) in
  let if_else_parser = seq (if_parser, rest_parser) in
  map (fun (_, (e, (s1,(_, s2)))) -> ((If (e, s1, s2)), dummy_pos)) if_else_parser
(* Handles if-else, going into a cstmt in the if statement *)
and make_if_parser (():unit) : (token, stmt) parser = 
  let if_parser = satisfy (fun t -> t == IF) in
  let rest_parser = seq(if_parser, lazy_seq(lazy (make_exp_parser ()), lazy (make_stmt_parser ()))) in
