...
let rec make_exp_parser (():unit) : (token, exp) parser =
  ...
  let rest_parser = seq (first_parser, make_add_subtract (*make_binop_rest ()*)) in
  let binop_parser = map (fun (e1, (op, e2)) -> (Binop (e1, op, e2), dummy_pos)) rest_parser in
  alts [binop_parser; first_parser]
and make_add_subtract (():unit) : (token, (binop * exp)) parser = 
  let add_subtract_parser = satisfy_opt (function
    PLUS -> Some Plus | MINUS -> Some Minus) in 
  lazy_seq (lazy add_subtract_parser, lazy (make_exp_parser ()))
and make_binop_rest (():unit) : (token, (binop * exp)) parser =
  let binop_op_parser = satisfy_opt (function 
    PLUS -> Some Plus | MINUS -> Some Minus | STAR -> Some Times | SLASH -> Some Div |
