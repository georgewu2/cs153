            ...
            | Call (e, vars) -> 
	            let caller_prep = 
	            	...
	            	then 
	            		...
	            		| _ -> []
	            in 
	            caller_prep @ Add(sp, sp, Immed (-16l))::Jal (func_name 

                e)::Add(sp, sp, Immed(16l))::[]
    in 
...
let compile_func ((Fn f):Ast.func) : inst list = 
	...
	let make_prologue (():unit) : inst list = 
		Label(func_name f.name)::allocate_word::Sw(ra, sp, 0l)::allocate_word::Sw(fp, sp, 0l)::
